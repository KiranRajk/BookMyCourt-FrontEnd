...
function Login({toggleBox}) {
    const [logCre, setLogCre] = useState({email:'', password:''})
       ...
       {/* logcre is a object
        ...
        const doLogin = () =>{
            axios.post(`${process.env.REACT_APP_BE_URL}/auth/login`, logCre)
            .then((res) => {
  ...
  return (
    <div className='signup-box d-flex flex-column p-3 rounded-3'  >
        ...
        <p>Not Registered? <span><i  onClick={()=>toggleBox('signup')}>Signup</i></span></p>
 
        <button className='btn btn-primary mt-3 ' onClick={doLogin}>Sign Up</button>
    </div>
  )
}
